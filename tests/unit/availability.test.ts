...
test("combined services use one eligible barber, summed duration, and one final buffer", () => {
  ...
  assert.ok(
    result.slots.every(
      (slot) =>
        slot.serviceEndsAt.getTime() - slot.startsAt.getTime() ===
          50 * 60_000 &&
        slot.protectedEndsAt.getTime() - slot.startsAt.getTime() ===
          60 * 60_000,
    ),
  );
});
...
test("eligibility is the active intersection of every selected service", () => {
  ...
  const unavailablePreference = calculatePublicAvailability(
    availabilityInput({
      serviceIds: ["service-demo-cejas"],
...
test("shop blocks and barber breaks remove capacity without affecting another barber", () => {
  ...
  for (const slot of result.slots) {
    ...
    assert.equal(
      intervalsOverlap(candidate, {
        startsAt: atLaPaz("11:00"),
        endsAt: atLaPaz("11:30"),
      }),
      false,
    );
  ...
  assert.ok(
    result.slots.some(
      (slot) =>
        slot.barberId === "barber-demo-sam" &&
        slot.startsAt.getTime() === atLaPaz("13:00").getTime(),
    ),
  );
});
...
test("the complete protected duration must fit before shop and barber schedule boundaries", () => {
  ...
  assert.ok(
    result.slots.every((slot) => slot.protectedEndsAt <= atLaPaz("10:00")),
  );
});
